...
def callback():
    ...
    print("callback(): cfg.OAUTH_TOKEN:{} and cfg.OAUTH_VERIFIER:{}".format(
    ...
    if cfg.OAUTH_TOKEN == cfg.REQUEST_OAUTH_TOKEN:
        print("callback(): cfg.OAUTH_TOKEN == cfg.REQUEST_OAUTH_TOKEN")
        # Step 3 - exchange request token for access token
        utils.get_oauth_access_token()

        if cfg.ACCESS_TOKEN and cfg.ACCESS_TOKEN_SECRET:
            print("callback(): SUCCESS Acquired accesstoken:{} and accesstokensecret:{}".format(
            ...
            print("callback():*****AUTHENTICATED********\n")
        else:
            print("callback(): FAILURE access token and secret not acquired")
            print("callback(): Acquired accesstoken:{} and accesstokensecret:{}".format(
                cfg.ACCESS_TOKEN, cfg.ACCESS_TOKEN_SECRET))
    else:
        print("callback(): cfg.OAUTH_TOKEN != cfg.REQUEST_OAUTH_TOKEN")
        print("callback(): cfg.OAUTH_TOKEN: %s" % cfg.OAUTH_TOKEN)
        print("callback(): cfg.REQUEST_OAUTH_TOKEN: %s" %
              cfg.REQUEST_OAUTH_TOKEN)
...
def view_data():
    ...
    print("view_data(): cfg.IS_AUTHENTICATED:{}, cfg.AUTHENTICATION_TRIED:{}".format(
